...
namespace JuFramework
{
	public abstract class Mesh
	{
		...
		{
			var minVertex = Vector3f.zero;
			var maxVertex = Vector3f.zero;
			...
			if (vertices != null)
			{
				...
				for (int i = 1; i < vertices.Length; ++i)
				{
					...
					maxVertex.x = Math.Max(minVertex.x, vertex.x);
					maxVertex.y = Math.Max(minVertex.y, vertex.y);
					maxVertex.z = Math.Max(minVertex.z, vertex.z);
				}
			}

			box.center = new Vector3f(
				(maxVertex.x - minVertex.x) / 2f - maxVertex.x,
				(maxVertex.y - minVertex.y) / 2f - maxVertex.y,
				(maxVertex.z - minVertex.z) / 2f - maxVertex.z
			);
			box.size = new Vector3f(maxVertex.x - minVertex.x, maxVertex.y - minVertex.y, maxVertex.z - minVertex.z);
		}
