...
namespace JuFramework
{
	public partial struct Color
	{
		...
		{
			rValue = Math.Clamp01(brightness);
			gValue = Math.Clamp01(brightness);
			bValue = Math.Clamp01(brightness);
			aValue = 1f;
		}
		...
		public static Color FromHSV(float hue, float saturation, float value, float alpha)
		{
			...
			return new Color(
				ColorExtension.RGBChannelFromHue(min, max, hue + (1 / 3f)),
				ColorExtension.RGBChannelFromHue(min, max, hue),
				ColorExtension.RGBChannelFromHue(min, max, hue - (1 / 3f)),
				alpha
			);
		}

		public static explicit operator Color(Vector4f vector)
		{
	...
	{
		public static float RGBChannelFromHue(float min, float max, float hue)
		{
			if (hue < 0)
			{
				hue += 1f;
		...
		public static void Clamp01(this Color color)
		{
			color.r = Math.Clamp01(color.r);
			color.g = Math.Clamp01(color.g);
			color.b = Math.Clamp01(color.b);
			color.a = Math.Clamp01(color.a);
		}
