...
public class PlatformCamera : Camera2D
{
	...
	public void Update()
	{
		...
		float cameraMovSpeed = 0.1f;
		if(Core.input.IsKeyHeld(KeyboardKey.LeftShift))		cameraMovSpeed = 0.01f;
		if(Core.input.IsKeyHeld(KeyboardKey.LeftControl))	cameraMovSpeed = 1f;
		...
		if(Core.input.IsKeyHeld(KeyboardKey.E))				Rotate(0.05f);
		if(Core.input.IsKeyHeld(KeyboardKey.Q))				Rotate(-0.05f);
		...
		if(Core.input.IsMouseButtonHeld(MouseButton.Middle))
		{
			...
			Translate((Vector2f.one / cameraMovSpeed) * -mouseDiff / GetZoom());
		}
	}
}
